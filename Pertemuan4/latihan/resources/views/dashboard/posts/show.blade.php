...
{{-- Di bagian <tbody> (didalam foreach) --}}
<td class="px-6 py-4">
    @if($post->image)
        <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->title }}" 
                class="w-16 h-16 rounded-lg object-cover border border-gray-200">
    @else
        <img src="{{ asset('images/preview.jpg') }}" alt="Image preview" 
                class="w-16 h-16 rounded-lg object-cover border border-gray-200 bg-gray-100">
    @endif
</td>